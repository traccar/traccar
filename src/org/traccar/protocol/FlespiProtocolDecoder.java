...
public class FlespiProtocolDecoder extends BaseProtocolDecoder {
    ...
    protected Object decode(
        ...
        JsonArray result = Json.createReader(new StringReader(request.getContent().toString(StandardCharsets.UTF_8)))
                .readArray();
        Map<Long, List> unitsPositions = new HashMap<>();
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            if (unitsPositions.get(deviceSession.getDeviceId()) == null) {
                unitsPositions.put(deviceSession.getDeviceId(), new LinkedList<Position>());
            }
            Position position = new Position();
            position.setDeviceId(deviceSession.getDeviceId());
            decodePosition(message, position);
            unitsPositions.get(deviceSession.getDeviceId()).add(position);
        }

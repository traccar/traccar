...
public class TollEventHandler extends BaseEventHandler{
    ...
    public void onPosition(Position position, Callback callback) {
        ...
        TollRouteProcessor.updateState(state,position,positionIsToll,positionTollRef,positionTollName, minimalDuration);
        if (state.isChanged()) {
            state.toDevice(device);
            try {
                storage.updateObject(device, new Request(
                        new Columns.Include("tollrouteState", "tollrouteTime"),

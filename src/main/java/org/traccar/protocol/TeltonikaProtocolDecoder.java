...
public class TeltonikaProtocolDecoder extends BaseProtocolDecoder {
    ...
    private void decodeCell(
        ...
        if (position.hasAttribute(mncKey) && position.hasAttribute(lacKey) && position.hasAttribute(cidKey)) {
            CellTower cellTower = CellTower.from(
                    ...
                    ((Number) position.removeAttribute(mncKey)).intValue(),
                    ((Number) position.removeAttribute(lacKey)).intValue(),
                    ((Number) position.removeAttribute(cidKey)).longValue());
            cellTower.setSignalStrength(((Number) position.removeAttribute(rssiKey)).intValue());
    ...
    private void decodeNetwork(Position position, String model) {
        if ("TAT100".equals(model)) {
            ...
            Integer cid2g = (Integer) position.removeAttribute("cid2g");
            Long cid4g = (Long) position.removeAttribute("cid4g");
            Integer lac = (Integer) position.removeAttribute("lac");

 ...
 * Copyright 2025 - 2025 Christina Maus (minnie@maus.xyz)
 *
 */
...
public class DcjbProtocolDecoder extends BaseMqttProtocolDecoder {
	...
	private Position parseInstanceGNSS(
		Position position,
		String name,
		JsonNode instanceValues
	) throws Exception {
	...
	private Position parseDataBlock(
		...
		/* The UID is ignored here, since the device management is done
		 * by Traccar on a higher level. We have to adhere to that so far!
		 */
		...
		if(blockInstances != null && blockInstances.isArray()){
			for(JsonNode blockInstance : blockInstances) {
				...
				/* The datetime is ignored, since only a single time can
				 * be set for each position in the Traccar system.
				 */
				
				/* Name */
				JsonNode instanceName = blockInstance.get("name");
				String name;
				if(instanceName == null || instanceName.isNull()){
					name = "";
				}
				else{
					name = instanceName.asText();
				}
	...
	protected Object decode(
		...
		try {
			...
			throw new Exception("Invalid version identifier.");
		}
			...
			if(packetData != null) {
				if(packetData.isArray()) {
					for(JsonNode dataBlock : packetData) {
						positions.add(parseDataBlock(
							deviceSession, version, datetime, dataBlock
						));
					}
				}
				else {

...
public class Gl200TextProtocolDecoderTest extends ProtocolTest {
    ...
    public void testDecode() throws Exception {
        ...
        verify(decoder, buffer(
                "+RESP:GTFRI,FE1712,015700000310332,,11308,10,1,1,0.0,0,1393.0,-111.891047,40.760780,20260922234154,0310,0410,4E20,0B4F2A01,,129297.4,02792:32:17,68,110000,,,20260922234153,F646$"),
                position().attribute(Position.KEY_POWER, 11.308));

        verify(decoder, buffer(
                "+RESP:GTFRI,DC0103,862170019025640,,12000,10,1,1,4.3,92,70.0,121.354335,31.222073,20140511093254,0460,0003,18D8,014B,00,2000.0,12345:12:34,5000,,80,210100,12500,,,20140511093255,11F0$"),
                position().attribute(Position.KEY_BATTERY_LEVEL, 80));

        verify(decoder, buffer(
                "+RESP:GTERI,DC0103,862170019025640,,00000000,12000,10,1,1,4.3,92,70.0,121.354335,31.222073,20140511093254,0460,0003,18D8,014B,00,2000.0,12345:12:34,5000,,80,210100,12500,20140511093255,11F0$"),
                position().attribute(Position.KEY_BATTERY_LEVEL, 80));

        verify(decoder, buffer(
                "+RESP:GTFRI,310903,862170019025640,,12000,10,1,1,4.3,92,70.0,121.354335,31.222073,20140511093254,0460,0003,18D8,014B,00,2000.0,12345:12:34,5000,,80,210100,,,,20140511093255,11F0$"),
                position().attribute(Position.KEY_BATTERY_LEVEL, 80));

        verify(decoder, buffer(
                "+RESP:GTERI,310903,862170019025640,,00000000,12000,10,1,1,4.3,92,70.0,121.354335,31.222073,20140511093254,0460,0003,18D8,014B,00,2000.0,12345:12:34,5000,80,210100,0,20140511093255,11F0$"),
                position().attribute(Position.KEY_BATTERY_LEVEL, 80));

        verify(decoder, buffer(
                "+RESP:GTFRI,F10310,868446036599153,gv350m,13242,10,1,1,0.0,0,231.2,114.015734,22.537908,20190823013021,0460,0001,253D,AEC3,,0.0,,,100,110000,,,20190823093022,0F76$"),
                position().attribute(Position.KEY_BATTERY_LEVEL, 100));

        verify(decoder, buffer(
                "+RESP:GTCAN,740B05,862524061506735,GV350CEU,10,1,FFBFFFFF,,1,H3700440,,0,0,76,,L25.50,,0,,0.00,0.37,,,8080,0040,00,00,0.00,0.00,0,3,2,699,0,B3FFFFFF,,,,,,,,,0,0,0,0.37,0.00,0.00,0,,,,,,0000,0,0,,,,0,,1,,17FC7,,,,,,,,,,,,,,,,1,0.0,0,2325.4,38.758242,8.999264,20260630101409,0636,0001,2BEB,01B9F304,11,12,0,20260630131409,F004$"),
                position().attribute(Position.KEY_FUEL, 25.5));

        verify(decoder, buffer(
                "+RESP:GTCAN,740100,863286020798295,GV350CEU-gl8-test-sandy,0,1,E07FFFFF,,2,H410120,1714.50,685,2,87,,P83.60,,0,556.89,329.88,227.01,345.96,,,0010,,00,20.08,15.25,007FFFFF,,,,,,,,,280724,584579,62,304.92,0.48,0.04,36232,,,,,,0000,,,,,0,3.4,145,11.0,121.362735,31.206570,20160816023617,0460,0000,1823,2043,00,20160816103618,1316$"),
                position());
        ...
        verify(decoder, buffer(
                ...
                position().attribute("tag1Id", "D325C2B2A2F8"));
        ...
        verify(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute(Position.KEY_CHARGE, null));
        ...
        verify(decoder, buffer(
                "+RESP:GTDAT,270B01,867162027893742,gv300w,1,,,>I:5014|DB|607|0|0.00|0.00|0.00|0|0|0|0|1679|4|<,0,37.6,184,1940.4,-101.041169,22.143452,20240726234344,0334,0020,2F03,BDB2E4B,00,,,,,20240726234346,4CFE$"),
                position().attribute("data", ">I:5014|DB|607|0|0.00|0.00|0.00|0|0|0|0|1679|4|<"));

        verify(decoder, buffer(
                "+RESP:GTPNA,710402,867292060140006,,2,20250516120427,0121$"),
                position(Checks.ATTRIBUTES).attribute(Position.KEY_BATTERY_LEVEL, null));

        verify(decoder, buffer(
                "+RESP:GTERI,F10415,862599050680717,GV350M,00000001,,50,1,1,0.0,274,2963.1,-78.691767,-0.951295,20250512214226,,,,,,0.0,,,100,210100,,1,1,1,01DF,20250512214226,009D$"),
                position().attribute(Position.KEY_FUEL, 479));

        verify(decoder, buffer(
                "+RESP:GTERI,6E0A03,868589060745174,,00000100,,10,1,1,0.0,0,1509.0,-90.544928,14.584461,20250307182819,0704,0001,13A7,000B60AB,01,12,358.6,,,,,100,1A0000,0,2,00,6,5,0E7109BB,283F,SENTEMP1,7805412CF2DD,1,3466,24,36,24.91,100,01,6,2,,083F,SENTEMP2,7805412CF25C,0,,,,,20250307182820,1ABC$"),
                position());

        verify(decoder, buffer(
                "+BUFF:GTIGF,840502,867963063190000,,0,0,,,,,,,0268,0006,22C4,13E5,00,,,20250116170541,22D0$"),
                position(Checks.ATTRIBUTES));

        verify(decoder, buffer(
                "+BUFF:GTERI,6E0A03,868589060742171,,00000005,,10,1,1,0.0,0,225.6,-90.795241,14.261515,20250301071037,0704,0001,10B9,00105915,01,12,0.0,,F0,F0,,100,1A0201,0,,0,0,20250301071038,062C$"),
                position());

        verify(decoder, buffer(
                "+RESP:GTERI,6E0A03,868589060076166,,00000100,,10,1,1,0.1,0,1510.4,-90.544955,14.584449,20250228072652,0704,0001,13A7,000B60AB,01,10,0.0,,,F0,,100,210100,0,1,00,6,5,,283F,SENTEMP1,7805412CF2DD,0,,,,,,20250228072654,3F94$"),
                position());
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_ALARM, Position.ALARM_CORNERING));
        ...
        verify(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute("adBlueLevel", 100.0));
        ...
        verify(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute("power2", 13.985));
        ...
        verify(decoder, buffer(
                "+RESP:GTLBS,660400,863574046265138,GL50BLITE,0,0,92,,2,,,0000,0204,0008,0cee,b951,18,,0204,0008,0cee,cd83,15,,0204,0008,0cee,3994,13,,,,,,,,,,,,,,,,,,,,0204,0008,0cee,d071,11,,20240826085102,00C6$"),
                position(Checks.ATTRIBUTES));

        verify(decoder, buffer(
                "+RESP:GTERI,F18501,862599050498599,GV350M_HHP,00000002,,51,1,1,18.9,140,319.5,46.586093,28.985816,20240817120402,0420,0001,03DA,AC81,,0.0,,,0,220100,,0,20240817120401,7860$"),
                position());

        verify(decoder, buffer(
                "+RESP:GTERI,F18501,862599050498599,GV350M_HHP,00000002,,10,1,1,0.0,0,186.5,47.711072,27.555913,20240816172311,0420,0004,2F51,4679,,0.0,,,100,110000,,0,20240816172311,BB1C$"),
                position());
        ...
        verify(decoder, buffer(
                "+RESP:GTERI,358803,862364030335589,gv200,00000002,,10,1,1,42.5,132,2150.4,43.516551,17.661988,20240818171932,0420,0004,F23D,31EF,00,17363.6,,,91,,00,00,2,0,20240818171940,91F1$"),
                position());

        verify(decoder, buffer(
                "+BUFF:GTERI,358803,862364030181926,gv200,00000011,,51,1,1,21.0,98,30.2,38.249517,24.008269,20240817131401,0420,0004,75A0,412C,00,84795.4,,,,,01,00,0,,20240817131358,F127$"),
                position());
        ...
        verify(decoder, buffer(
                ...
                position().attribute("accessoryVoltage", 3.065));
        ...
        verify(decoder, buffer(
                ...
                position().attribute("accessoryVoltage", 2.934));
        ...
        verify(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute(Position.KEY_FUEL, 94.0));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_FUEL, 153));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.PREFIX_TEMP + 1, 25.0));
        ...
        decoder.setModelOverride("ATWG7");
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_BATTERY_LEVEL, 85));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_BATTERY_LEVEL, 79));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.PREFIX_IN + 1, false));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_BATTERY_LEVEL, 9));
        ...
        verify(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute(Position.KEY_ALARM, Position.ALARM_POWER_OFF));

        verify(decoder, buffer(
                "+RESP:GTPNA,F50201,866425030235982,GL300M,20190208124909,0BD5$"),
                position(Checks.ATTRIBUTES).attribute(Position.KEY_ALARM, Position.ALARM_POWER_ON));
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_BATTERY_LEVEL, 86));
        ...
        decoder.setModelOverride("ATPLUS");
        ...
        verify(decoder, buffer(
                ...
                position().attribute(Position.KEY_ODOMETER, 2000000.0));
        ...
        decoder.setModelOverride("GV65L");
        ...
        verify(decoder, buffer(
                "+RESP:GTINF,6E0202,868589060187625,RA82,11,89883030000091225018,41,0,1,12349,,4.15,0,1,0,0,20240328231013,0,0,0,0,00,00,+0000,0,20240328231015,7D4F"),
                position(Checks.ATTRIBUTES).attribute(Position.KEY_POWER, 12.349));

        verify(decoder, buffer(
                "+RESP:GTINF,6E0202,868589060187625,RA82,11,89883030000091225018,41,0,1,12349,,4.15,0,1,0,0,20240328231013,0,0,0,0,00,00,+0000,0,20240328231015,7D4F"),
                position(Checks.ATTRIBUTES).attribute(Position.PREFIX_ADC + 3, 0));

...
public class Pt502ProtocolDecoderTest extends ProtocolTest {
    ...
    public void testDecode() throws Exception {
        ...
        verifyDecode(decoder, buffer(
                "$POS,0000001,112742.000,A,0900.9050,N,03845.0609,E,13.12,212.34,290421,,,A/00010,00000/0,0,0,0/250161300//fd7//"),
                position().attribute(Position.KEY_INPUT, 2));
        ...
        verifyDecode(decoder, buffer(
                ...
                position().attribute(Position.KEY_ALARM, Position.ALARM_POWER_CUT));

        verifyDecode(decoder, buffer(
                "$POS,216769295715,163237.000,A,3258.1738,S,02755.4350,E,0.00,215.88,100915,,,A/0000,0//232300//5b3/"),
                position().location("2015-09-10T16:32:37.000Z", true, -32.96956, 27.92392));

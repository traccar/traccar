...
public class H02ProtocolDecoderTest extends ProtocolTest {
    ...
    public void testDecode() throws Exception {
        ...
        verifyDecode(decoder, buffer(
                "*HQ,1234567890,V8,112806,A,3832.7803,N,9.9068,W,64.14,67,310826,FFFFF9FF,214,3,18106,9261854,20,21,126,0#"),
                position().location("2026-08-31T11:28:06.000Z", true, 38.54634, -0.16511));

        verifyDecode(decoder, buffer(
                "*HQ,9001000002,V8,213945,A,3542.2043,N,38.6508,W,0.00,170,221025,FBFFF9FF,0,0,0,0,22,31,126,0#"),
                position().location("2025-10-22T21:39:45.000Z", true, 35.70340, -0.64418));
        ...
        verifyDecode(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute(Position.KEY_BATTERY_LEVEL, 100));
        ...
        verifyDecode(decoder, buffer(
                ...
                position().location("2015-12-22T16:45:49.000Z", true, 9.93978, -84.11178));
        ...
        verifyDecode(decoder, buffer(
                ...
                position().location("2015-10-03T05:04:18.000Z", false, 28.46021, 77.06414));

        verifyDecode(decoder, buffer(
                "*HQ,1451316409,V1,030149,A,-23-29.0095,S,-46-51.5852,W,2.4,065,070315,FFFFFFFF#"),
                position().location("2015-03-07T03:01:49.000Z", true, -23.48349, -46.85975));
        ...
        verifyDecode(decoder, binary(
                ...
                position().location("2015-08-31T21:53:21.000Z", true, 4.69898, -74.06971));
        ...
        verifyDecode(decoder, buffer(
                ...
                position(Checks.ATTRIBUTES).attribute(Position.KEY_RESULT, "ST906(70SACD)_TQ_V_2.0 2024/06/07\nID:5226073533\nIP:1.2.3.4 5013\nUT:30,30,300\nVOLT:12.9V\nAPN:internet.example.com\nGPS:A-24-23\nGSM:26"));
    ...
    public void testDecodeStatus() throws Exception {
        ...
        verifyDecode(decoder, buffer(
                "*HQ,2705171109,V1,213324,A,5002.5849,N,01433.7822,E,0.00,000,140613,FFFFFFFF#"),
                position().attribute(Position.KEY_STATUS, 0xFFFFFFFFL));

        verifyDecode(decoder, binary(
                "2441091144271222470112142233983006114026520E000000FFFFFBFFFF0014060000000001CC00262B0F170A"),
                position().attribute(Position.KEY_STATUS, 0xFFFFFBFFL));

...
public class class_tini_batch_offline_enh_io 
{
	...
	public class_tini_batch_offline_enh_io(byte type, int seq, int boxId, String data)throws Exception
	{
		this.type = type;
		this.seq = seq;
		this.boxId = boxId;
	...
	public void manageData()throws Exception
	{
		...
		try
		{
			...
			//System.out.println("box_id: "+boxId+"\tDATA: "+data);
			//System.out.println("box_id: "+boxId+"\tDATA Lenght: "+data.length());
			...
			if(crcFromUDP.equalsIgnoreCase(crcCalculate))
			{
				...
				// New byte
				//System.out.println("box_id: "+boxId+"\tData SUB:"+data.substring(62));
				String event1 = data.substring(62,64);
				String event2 = data.substring(64,66);
				String event3 = data.substring(66,68);
				String crc = data.substring(68,70);
				...
				String ftime_gps = objDecode.findDatetimeOffline(Gateway_HexConvert.HextoInteger(datetime),boxId);			
				...
				// new decode
				String sensor1 = Gateway_HexConvert.HextoBinary(event1);
				//System.out.println("box_id: "+boxId+"\tEvent1:"+event1+"\tBinary:"+sensor1);
				String sensor2 = Gateway_HexConvert.HextoBinary(event2);
				//System.out.println("box_id: "+boxId+"\tEvent2:"+event2+"\tBinary:"+sensor2);
				String extension = Gateway_HexConvert.HextoBinary(event3);
				//System.out.println("box_id: "+boxId+"\tEvent3:"+event3+"\tBinary:"+extension);			
			    String data_sensor = sensor1+sensor2+extension;
				...
				msgFile = objUdp.SortDataMessage("1",boxId,ftime_gps,ftime_sv,flat,flon
				...
				if(udp_return[0].equalsIgnoreCase("Socket timeout 5 second"))
				{
					...
					Gateway_CreateLogFile.createBatchOffline("["+udp_return[1]+"] UDP : "+msgFile,boxId);
					Gateway_CreateLogFile.createBatchOffline("["+udp_return[2]+"] Ack : "+udp_return[0],boxId);
				...
				this.udp_return=udp_return[0]; // Set Return ���
...
//				System.out.println(msgFile);
				Gateway_CreateLogFile.createBatchOffline(msgFile,boxId);

			}
			else
			{
				msgFile = "ERROR, CRC of Message S (tini batch offline enh i/o) error"

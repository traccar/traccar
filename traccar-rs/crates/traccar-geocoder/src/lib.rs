...
pub enum GeocoderError {
    #[error("Geocoder request error: {0}")]
    Request(String),
    #[error("Parse error: {0}")]
    Parse(String),
...
impl From<reqwest::Error> for GeocoderError {
    fn from(e: reqwest::Error) -> Self {
        GeocoderError::Request(e.to_string())
    }
}

/// Trait for reverse geocoding (coordinates to address).
#[async_trait]
...
/// Factory that creates the appropriate geocoder based on configuration.
pub struct GeocoderFactory;

impl GeocoderFactory {
    pub fn create(geocoder_type: &str, url: Option<&str>, key: Option<&str>) -> Box<dyn Geocoder> {
        match geocoder_type {
...
pub fn create_geocoder(geocoder_type: &str, url: Option<&str>, key: Option<&str>) -> Box<dyn Geocoder> {
    GeocoderFactory::create(geocoder_type, url, key)
}
